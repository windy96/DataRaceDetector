...
float pspeedy(Points *points, float z, long *kcenter, int pid, pthread_barrier_t* barrier)
{
  ...
  if( pid==0 )   {
    *kcenter = 1;
    costs = (double*)malloc_pmc(sizeof(double)*nproc);
  }
  ...
  if( pid != 0 ) { // we are not the master threads. we wait until a center is opened.
    while(1) {
      ...
      // windy
      if (open) pthread_cond_wait_null(&cond, &mutex);
      else
...
double pgain(long x, Points *points, double z, long int *numcenters, int pid, pthread_barrier_t* barrier)
{
  ...
  if( pid==0 )    { 
    work_mem = (double*) malloc_pmc(stride*(nproc+1)*sizeof(double));
    gl_cost_of_opening_x = 0;
    gl_number_of_centers_to_close = 0;
  }
...
#endif
{
  ...
  *feasible = (int *)malloc_pmc(numfeasible*sizeof(int));
...
#else
  accumweight= (float*)malloc_pmc(sizeof(float)*points->num);
#endif
	       ...
	       int pid, pthread_barrier_t* barrier )
{
  ...
  if( pid==0 ) hizs = (double*)calloc_pmc(nproc,sizeof(double));
  hiz = loz = 0.0;
  long numberOfPoints = points->num;
  long ptDimension = points->dim;
...
void copycenters(Points *points, Points* centers, long* centerIDs, long offset)
{
  ...
  bool *is_a_median = (bool *) calloc_pmc(points->num, sizeof(bool));
...
void outcenterIDs( Points* centers, long* centerIDs, char* outfile ) {
  ...
  int* is_a_median = (int*)calloc_pmc( sizeof(int), centers->num );
		    ...
		    long chunksize, long centersize, char* outfile )
{
...
#else
  float* block = (float*)malloc_pmc( chunksize*dim*sizeof(float) );
...
#else
    (Point *)malloc_pmc(chunksize*sizeof(Point));
#endif
  ...
  while(1) {
    ...
    is_center = (bool*)calloc_pmc(points.num,sizeof(bool));
    center_table = (int*)memoryInt.allocate(points.num*sizeof(int));
#else
    switch_membership = (bool*)malloc_pmc(points.num*sizeof(bool));
    is_center = (bool*)calloc_pmc(points.num,sizeof(bool));
    center_table = (int*)malloc_pmc(points.num*sizeof(int));
#endif
...
#else
    free_pmc(is_center);
    free_pmc(switch_membership);
    free_pmc(center_table);
#endif
  ...
  is_center = (bool*)calloc_pmc(centers.num,sizeof(bool));
  center_table = (int*)memoryInt.allocate(centers.num*sizeof(int));
#else
  switch_membership = (bool*)malloc_pmc(centers.num*sizeof(bool));
  is_center = (bool*)calloc_pmc(centers.num,sizeof(bool));
  center_table = (int*)malloc_pmc(centers.num*sizeof(int));
#endif
...
*/
  ...
  scanf("%ld %ld %ld %ld %ld %ld %s %s %d", &kmin, &kmax, &dim, &n, &chunksize, &clustersize, infilename, outfilename, &nproc);
  //scanf("%ld %ld %ld %ld %ld %ld %ld", &kmin, &kmax, &dim, &n, &chunksize, &clustersize, infilename, outfilename, &nproc);
