...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
   ...
   if (argc != 4)
    ...
    data = (OptionData*)malloc_pmc(numOptions*sizeof(OptionData));
    prices = (fptype*)malloc_pmc(numOptions*sizeof(fptype));
...
#define LINESIZE 64
    ...
    buffer = (fptype *) malloc_pmc(5 * numOptions * sizeof(fptype) + PAD);
    ...
    buffer2 = (int *) malloc_pmc(numOptions * sizeof(fptype) + PAD);
...
#else
    int *tids;
    tids = (int *) malloc_pmc (nThreads * sizeof(int));

// windy
// changed to avoid false warning
    for(i=0; i<nThreads; i++) 
        tids[i]=i;
    for(i=0; i<nThreads; i++) {
